...
A beginner-friendly single-page React app built with Vite and **React 19**. It demonstrates
foundational React concepts—state, props, effects, context, reducers, memoization, refs, and
controlled forms—with concise, interactive examples.
...
### Latest install attempt (CI sandbox)

`npm install` was retried in this environment and continues to fail with `403 Forbidden` responses
from the public npm registry for `@vitejs/plugin-react`. This appears to be a sandbox network
restriction rather than a project issue. If you have normal internet access, the install should
work after applying the registry/proxy steps above.
